...
- **TypeScript Types over Interfaces:** Prefer using `type` rather than `interface` when defining TypeScript types, unless an interface is strictly necessary (e.g., for declaration merging).
- **Prisma Types:** Do not duplicate types generated by Prisma (e.g., in a shared `types` package). Export the required types directly from the backend (e.g., `apps/backend/src/types.ts`) and import them across the monorepo using type-only imports to prevent redundant declarations and bundle bloat.
